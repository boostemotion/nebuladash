  ...
  <div class="flex flex-col gap-2 p-4 text-sm">
    ...
    <template v-if="hasVisibleProxyStyleItems">
      ...
      <div class="settings-grid">
        ...
        <div
          v-if="isVisibleNodeClusterVariants"
          class="setting-item col-span-full"
        >
          <div class="setting-item-label">
            {{ $t('nodeClusterVariants') }}
          </div>
          <TextInput
            class="flex-2"
            v-model="nodeClusterVariantsInput"
            :clearable="true"
            :placeholder="$t('nodeClusterVariantsPlaceholder')"
          />
        </div>
      </div>
    </template>
...
const isVisibleNodeClusterVariants = useIsSettingVisible(k.nodeClusterVariants)
...
const hasVisibleProxyStyleItems = computed(() => {
  return (
    ...
    isVisibleNodeClusterVariants.value
  )
})

const nodeClusterVariantsInput = computed({
  get: () => nodeClusterVariants.value.join(', '),
  set: (value: string) => {
    nodeClusterVariants.value = value
      .split(/[\n,，、;；]+/)
      .map((item) => item.trim())
      .filter(Boolean)
  },
