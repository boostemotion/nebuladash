...
const getRenderProxies = (proxies: string[], groupName?: string) => {
  ...
  if (proxiesFilter.value) {
    const filters = splitSearchTerms(proxiesFilter.value)
    const groupNameMatchesFilter =
      groupName &&
      filters.every((filter) => {
        const variants = getSearchTermVariants(filter)

        return variants.some((variant) => matchesSearchTerm(groupName, variant))
      })

    if (groupNameMatchesFilter) {
      return sortProxies(proxies, latencyMap, groupName)
    }
...
const sortProxies = (proxies: string[], latencyMap: Map<string, number>, groupName?: string) => {
