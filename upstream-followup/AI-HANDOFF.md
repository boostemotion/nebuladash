...
2. 不依赖路由器实测时，优先补 Provider 缓存旧数据提示、错误通知去重、后端切换和缓存清理测试。
3. 固化 Release 发布前检查清单或脚本。
4. 路由器空闲后，再做真实 OpenClash / Mihomo Provider 超时、缓存和后端切换回归。
5. 多后端标题、后端 uptime、代理图标映射等非视觉核心信息展示按需评估。
