...
> （2026-06-24）整理。本文是后续维护的总入口，不替代已有 `PLAN.md`、`SOP.md`
> 和 `UPSTREAM-FEATURES.md`，后续应逐步将过期内容合并到本文。
...
当前 NebulaDash 包版本为 `2.8.0-nebula.1`，上游代码基线仍来自 Zashboard
...
缓存用于先显示上次有效数据，再在后台更新，降低慢后端带来的空白等待感。

关键文件：

- `src/store/proxies.ts`

后续需要补充缓存时间、后端隔离和失效策略，避免不同后端之间错误复用数据。
...
1. 设置小修：select 外观、设置 label 宽度、代理设置下拉宽度
2. 连接页轻量信息：多后端标题、后端 uptime、连接延迟统计
3. 搜索模式：节点搜索和显式正则模式
4. Provider 搜索增强
5. 规则卡片动画
...
- 对当前 OpenClash / Mihomo 场景收益有限
- 会明显扩大构建体积、权限范围和维护面
- 会触碰大量设置、路由、API 和页面结构
- 容易掩盖真正需要解决的 Provider 慢请求问题
...
- 建立稳定的 NebulaDash Release 和构建产物发布流程
- 确认 OpenClash 的 UI 下载地址指向 NebulaDash 后，再决定是否保留自动升级
...
type ProviderCacheMeta = {
  fetchedAt: number
  durationMs: number
  status: 'fresh' | 'cached' | 'timeout' | 'error'
}
```
...
1. 在 OpenClash 路由器环境复测 Provider 超时、缓存和后端切换
2. 移植代理页空引用和移动端视口修复
3. 建立 NebulaDash Release 构建与发布流程
4. 再进入搜索模式扩展

在以上稳定性任务完成前，不建议开始代理文件夹或 Zashboard 3.x 视觉重构。
