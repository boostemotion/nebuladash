...
当前 NebulaDash 包版本为 `2.8.0-nebula.4.2.0`，上游代码基线仍来自 Zashboard
...
- 可跟进：设置项 label/select 宽度修复、select 默认外观修复、连接状态展示这类低风险体验补丁。
...
- 后端 uptime 展示：上游 `bed57f21` 依赖 sing-box native gRPC `GetStartedAt`，当前
  OpenClash / Mihomo 主线没有通用 startedAt 来源，不做空 UI 移植。
...
1. 路由器空闲后做真实 OpenClash / Mihomo Provider 超时、缓存和后端切换回归。
2. 多后端标题、连接状态延迟统计、代理图标映射等非视觉核心信息展示按需评估。
3. 自管理更新器 rollback 实测降为低优先级，后续改更新器核心逻辑时再做破坏性实测。
...
- 当前工作区版本已推进到 `2.8.0-nebula.4.2.0`；仍保持 Zashboard `2.8.0` 基线，未冒充上游 3.x。
- 发布前检查已固化为 `pnpm release:check`，用于校验 Nebula 版本格式和 tag/package 一致性。
- `v2.8.0-nebula.4.2.0` 作为下一次发布目标；发布后需确认 `latest/download/dist.zip` 跳转到该版本。
...
- rollback 暂不作为近期优先项。已有 A/B 分区和 SSH 手动修复路径，实际使用频率低；后续改更新器核心逻辑时再补真实路由器破坏性测试。
...
- 多后端标题、连接状态延迟统计等非视觉核心信息展示
- 代理图标映射增强
- 代理文件夹
- 设置同步
- 路由器更新器 rollback 回归
- 后端 uptime 展示：仅在 OpenClash / Mihomo 有可靠启动时间来源，或后续接入 sing-box native
  `GetStartedAt` 后再评估
...
1. 路由器空闲后，再做真实 OpenClash / Mihomo Provider 超时、缓存和后端切换回归。
2. 不依赖路由器的下一步，可评估多后端标题、连接状态延迟统计或代理图标映射的小范围适配。

rollback 实测保留为低优先级验证项；在核心稳定性任务完成前，不建议开始代理文件夹、搜索模式或 Zashboard 3.x 视觉重构。
