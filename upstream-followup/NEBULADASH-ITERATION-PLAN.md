...
当前 NebulaDash 包版本为 `2.8.0-nebula.2`，上游代码基线仍来自 Zashboard
...
- 本地上游代码基线：`2.8.0`
- 当前 NebulaDash 包版本：`2.8.0-nebula.2`
- 官方最新稳定版：`3.11.0`
- 官方发布时间：2026-06-24
...
1. Release 发布前检查清单或脚本固化
2. 多后端标题、后端 uptime 等非视觉核心信息展示按需评估
3. 路由器空闲后做真实 OpenClash / Mihomo Provider 超时、缓存和后端切换回归
...
- 当前开发线版本已推进到 `2.8.0-nebula.2`；仍保持 Zashboard `2.8.0` 基线，未冒充上游 3.x。
...
- 更新器只接受 `status`、`update`、`rollback` 三个动作，并要求 `X-NebulaDash-Token`。
...
- 路由器实际验证前，不要宣称该脚本已经在 OpenWrt 生产环境完成回归。
...
1. 在不依赖路由器实测的前提下，固化 Release 发布前检查清单或脚本。
