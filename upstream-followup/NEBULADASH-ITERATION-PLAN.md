...
1. Provider 缓存旧数据提示和元数据整理
2. Provider 请求错误通知去重
3. 后端切换、缓存隔离和删除后端缓存清理的测试补强
4. Release 发布前检查清单或脚本固化
5. 多后端标题、后端 uptime 等非视觉核心信息展示按需评估
...
1. 先提交当前文档整理和 Provider 搜索增强，形成稳定基线。
2. 在不依赖路由器实测的前提下，补 Provider 缓存、错误通知、后端切换和发布流程的测试与文档。
